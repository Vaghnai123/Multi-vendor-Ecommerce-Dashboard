...
import { get_blog_by_id, update_blog, messageClear } from '../../store/Reducers/blogReducer';
...
const EditBlog = () => {
    ...
    const { blog, loader, successMessage, errorMessage } = useSelector(state => state.blog);
    ...
    useEffect(() => {
        if (blogId) {
            dispatch(get_blog_by_id(blogId));
        }
    }, [blogId, dispatch]);
    useEffect(() => {
    ...
    const update = (e) => {
        ...
        formData.append('old_image', blog.image); 
        
        if (image) {
            formData.append('image', image);
        }

        dispatch(update_blog({ formData, blogId }));
    };
    ...
    return (
        <div className='px-2 lg:px-7 pt-5 h-full bg-[#cdcae9] min-h-screen'>
            ...
            <div className='w-full p-4 flex justify-between items-center'>
                 <h1 className='text-[#333] font-bold text-2xl'>Edit Blog</h1>
