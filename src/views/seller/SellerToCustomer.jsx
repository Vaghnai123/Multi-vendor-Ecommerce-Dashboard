...
const SellerToCustomer = () => {
    ...
    useEffect(() => {
        ...
        socket.on('activeCustomer', (customers) => {
            console.log("Active Customers from Server:", customers);
...
{
        customers.map((c,i) => <Link key={i} to={`/seller/dashboard/chat-customer/${c.fdId}`} className={`h-[60px] flex justify-start gap-2 items-center text-white px-2 py-2 rounded-md cursor-pointer bg-[#8288ed] mb-3`}>
                <div className='relative'>
                    <img className='w-[38px] h-[38px] border-white border-2 max-w-[38px] p-[2px] rounded-full' src="/image/demo.jpg" alt="" />
                    { activeCustomer.some(u => u.customerId === c.fdId) && <div className='w-[10px] h-[10px] bg-green-500 rounded-full absolute right-0 bottom-0'></div>
                    }
                </div>
    ...
    <div className='w-full md:w-[calc(100%-200px)] md:pl-4'>
        ...
        <div className='flex justify-between items-center'>
        {
             currentCustomer && <div className='flex justify-start items-center gap-3'>
                <div className='relative'>
                    <img className='w-[45px] h-[45px] border-green-500 border-2 max-w-[45px] p-[2px] rounded-full' src="/image/demo.jpg" alt="" />

                    { activeCustomer.some(u => u.customerId === currentCustomer.fdId) && <div className='w-[10px] h-[10px] bg-green-500 rounded-full absolute right-0 bottom-0'></div>
                    }
                </div>
...

            {
                customerId ? messages.map((m,i) => {
                    if (m.senderId === customerId) {
                        return(
                            <div key={i} ref={scrollRef} className='w-full flex justify-start items-center'>
                                <div className='flex justify-start items-start gap-2 md:px-3 py-2 max-w-full lg:max-w-[85%]'>
                                    <div>
                                        <img className='w-[38px] h-[38px] border-2 border-white rounded-full max-w-[38px] p-[3px]' src="/image/demo.jpg" alt="" />
                                    </div>
                        ...
                        <div className='w-[80px] h-[80px] bg-indigo-100 rounded-full flex justify-center items-center text-indigo-500 text-3xl'>
                            <AiOutlineMessage />
                        </div>
                            <span className='text-xl font-bold text-slate-700'>Select a Customer</span>
                    </div>
            }
            </div> 
        </div>
        ...
        <form className='flex gap-3' onSubmit={send}>
            ...
            <button disabled={customerId ? false : true} type="submit" className='bg-indigo-500 hover:bg-indigo-600 text-white w-12 h-12 rounded-full flex justify-center items-center shadow-lg hover:shadow-indigo-500/50 transition-all duration-300 flex-shrink-0'> 
                <IoSend size={22} />
            </button>
        </form>
